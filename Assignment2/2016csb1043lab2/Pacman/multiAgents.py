...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        #Get all food distance from pacman
        distFood = [util.manhattanDistance(newPos, pos) for pos in foodList]
        minGhost = float('Inf')

        #Get distance of all ghosts which are not scared, Scared ghosts are no danger to pacman
        for ghost in newGhostStates :
	        
	        if(ghost.scaredTimer <= 0):
	        
	            #Get minimum of distances
        ...
        #If ghost is not near just return sum of distance food else substract ghost distance so it moves away from it
        if(minGhost >= 3):
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def minValue(currentState, depth, agentIndex):
            ...
            #Get minimum of all legal actions of ghosts
            for action in actionGhosts:
            
                newState = currentState.generateSuccessor(agentIndex, action)
                
                #Call driver function for next action to call
        ...
        def maxValue(currentState, depth, agentIndex):
            ...
            #Get Max action of all legal actions
        ...
        def minMax(currentState, depth, agentIndex):
            ...
            #If cutoff is reached return evaluation function    
            if(self.depth == depth or currentState.isWin() or currentState.isLose()):
                return ("", self.evaluationFunction(currentState))
            
            #Select max action for pacman and min action for ghost
            if(agentIndex == 0):
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def minValue(currentState, depth, agentIndex, alpha, beta):
            ...
            #Get minaction for all legal actions
            for action in actionGhosts:
            
                newState = currentState.generateSuccessor(agentIndex, action)
                
                #Call driver code next legal action
                v_val = minMax(newState, depth, agentIndex+1, alpha, beta)  
                
                if(v_val[1] <= minValAction[1]):
                    minValAction = (action, v_val[1])
                
                #If we get value less than alpha prue the tree and return value    
                if(v_val[1] < alpha):
                    return minValAction
                
                #Else update value for that subtree of beta    
                if(v_val[1] < beta):
                    beta = v_val[1]
        ...
        def maxValue(currentState, depth, agentIndex, alpha, beta):
            ...
            for action in actionPacman:
                ...
                #prune the tree if value is greater than beta
                if(v_val[1] > beta):
                    return maxValAction
                
                #Update alpha for that subtree    
                if(v_val[1] > alpha):
                    alpha = v_val[1]
                    
            return maxValAction
        
        #Driver code to call min and max value function    
        def minMax(currentState, depth, agentIndex, alpha, beta):
            ...
            #For cutoff return evaluation function    
            if(self.depth == depth or currentState.isWin() or currentState.isLose()):
                return ("", self.evaluationFunction(currentState))
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def expectMinValue(currentState, depth, agentIndex):
            ...
            #Assign probablity as uniform 1/moves
            ...
            #For all legal action score = sum(prob*value)
            for action in actionGhosts:
        ...
        #Get max value of all actions of pacman    
        def maxValue(currentState, depth, agentIndex):
            ...
            #For all legal actions select max of it
            for action in actionPacman:
...
def betterEvaluationFunction(currentGameState):
    ...
    #Calculate distance from pacman to all fodd
    distFood = [util.manhattanDistance(newPos, pos) for pos in foodList]
    minGhost = float('Inf')

    #Get distance from scared gost same as reflex agent evaluation function
    for ghost in newGhostStates :
    ...
    #If ghost is far not need to substract ghost distance else give more weight to to ghost dist
    if(minGhost >= 3):
